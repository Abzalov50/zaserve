...
(defmacro with-http-body ((req ent
  ...
  (let ((g-req (gensym))
	...
	(g-body (gensym))
	(g-ent-comp-type (gensym)))
    (declare (ignorable g-old-request-reply-stream))
    `(let* ((,g-req ,req)
	   (,g-ent ,ent)
	    (,g-headers ,headers)
	    (,g-ent-comp-type (entity-compress-type ,ent)) 	   
	   #+(and allegro (version>= 6 0 pre-final 1))
	  ...
	  then (bulk-set-reply-headers ,g-req ,g-headers))
       ...
       (if* (not (member :omit-body (request-reply-strategy ,g-req) 
	    ...
	    then #+#.(cl:if net.aserve::*with-output-to-buffer-can-grow-array* '(and) '(or))
		    ...
		    then
		    (maybe-accumulate-log (:xmit-server-response-body "~s")
  ...
  (defparameter *fast-reply-headers*
	...
	(dolist (name '(:date
	  ...
	  (push (list name   ;; string name
		      ...
		      ;; accessor name
		      (read-from-string 
		       (format nil "request-reply-~a" name)))
		res))
	res))
...
(defun start (&key (port 80 port-p) 
		   ...
		   backlog
		(compress (or (and (member :zlib-deflate *features*) t)
			      #+(and sbcl salza2) (progn t)))
		)
  ;(format t "compress: ~A" compress) 
  ;; -exported-
  ;;
...
(defun start-lisp-thread-server (listeners max-listeners)
  ...
  (when (not (wserver-accept-thread *wserver*))
...
(defun get-body-input-stream (req encoding-table external-format)
  ...
  (if (request-request-body req)
	...
	(let ((compress-fn
	       #'(lambda (compressor)
		   (write-sequence
		    (salza2:compress-data stream compressor)
		    stream)
		   (setq str stream))))
	  (if (equal enc "gzip")
	      (funcall compress-fn 'salza2:gzip-compressor)
	      (funcall compress-fn 'salza2:deflate-compressor)))
	;;;;
	)
